...
cp /cdrom/fox-first-run.sh /target/home/${USERNAME}/fox-first-run.sh
chmod +x /target/home/${USERNAME}/fox-first-run.sh
chown -R 1000:1000 /target/home/${USERNAME}/fox-first-run.sh

# Create an autostart launcher for first-run script
mkdir -p /target/home/${USERNAME}/.config/autostart
cat << EOF > /target/home/${USERNAME}/.config/autostart/fox-first-run.desktop
[Desktop Entry]
Type=Application
Exec=/home/ubuntu/fox-first-run.sh
Hidden=false
NoDisplay=false
